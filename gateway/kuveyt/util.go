...
func accountFields(c model.BankCredentials) map[string]string {
	return map[string]string{
		...
		"CustomerId": c.Password, // Kuveyt: Password alanı Müşteri No (CustomerId)
		"UserName":   c.Username,
	}
}

func hashFieldsFrom(c model.BankCredentials, extra map[string]string) map[string]string {
	h := accountFields(c)
...
// parseHTMLForm enrollment yanıtındaki HTML 3D formunu çıkarır.
func parseHTMLForm(html string) (gateway string, inputs map[string]string, err error) {
	if !strings.Contains(strings.ToLower(html), "<form") {
		return "", nil, fmt.Errorf("kuveyt: HTML form bulunamadı")
	}
	m := formActionRe.FindStringSubmatch(html)
	if len(m) < 2 {
		return "", nil, fmt.Errorf("kuveyt: form action bulunamadı")
	}
